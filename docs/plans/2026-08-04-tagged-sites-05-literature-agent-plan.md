...
**Architecture:** A pipeline under `src/accessible_surfaceome/agents/tag_site/` with two **run modes** (spec §9): `benchmark` (gene + name only — scores the retrieval skill) and `production` (gene + name + computed `sequence`/`per_residue_topology` — sources display data and pins exact residues). Retrieval + LLM reuse the **project's existing stack, not MCP**: `tools/evidence_retrieval.py:evidence_retrieval(uniprot_acc, category)` for grounded snippets and `agents/surfaceome_v2/builders/_common.py:call_builder` for the Claude call. That step is not unit-testable; everything around it is: prompt assembly, output-schema validation, residue-junction verification (reuse Python parity of `verifyJunction`), EC classification, and the recall scorer. Emits via Plan 4's `emit_tag_sites_json` merge-writer (so literature + deterministic coexist per gene).

**Tech Stack:** Python 3, pytest, the repo's agent framework (`src/accessible_surfaceome/agents/`), the project retrieval stack (`tools/evidence_retrieval.py`, `tools/gene_literature.py`, `tools/gene_lookup.py`) + the shared Anthropic-SDK builder (`agents/surfaceome_v2/builders/_common.py:call_builder`), jsonschema. **No MCP.**
...
**Placeholder scan:** the retrieval + `call_builder` LLM invocation in Task 4 Step 1 is described, not coded, because it is inherently non-deterministic (network + model) — every deterministic surround (schema, prompt assembly, normalization/verification, scoring) is full TDD. This is the appropriate boundary.
...
**Dependency note:** the only runtime dependencies are an Anthropic API key/budget (`ANTHROPIC_API_KEY`, used by `call_builder`) and outbound network to EuropePMC/NCBI (cached + rate-limited via `tools/_shared`). **No MCP connectors.** When a gene has no retrievable evidence, `evidence_retrieval` returns an empty pack with an `empty_reason` — pass it through, don't fabricate.

...
## Implementation notes & deviations (2026-09-01, post-build + review)

Built, adversarially reviewed end-to-end, and merged to `dev` with these acknowledged, non-blocking trade-offs (canonical sites verified correct against the real xlsx data; all gates green):

- **Isoform matching is by offset-corrected protein length, not sequence identity.** The spec's identifier-resolution called for ENST→UniProt-isoform matching by protein sequence (`pct_identity`). media-2's Isoforms sheet carries only lengths — and those are the *HA-tagged construct* length — so the emitter matches on `tedman_len − per_gene_tag_offset` (offset = the sheet's own canonical-length echo minus the record's canonical length) against the record's `isoform_topologies`, requiring a **unique** exact-length hit (else the pin is dropped, never mis-assigned). Conservative and empirically clean (73 pins / 49 genes; 0 duplicate isoform_ids; 103 dropped for no-unique-match or no isoform_topologies). Residual risk: an alt isoform whose own construct offset differs from the canonical's could, post-subtraction, coincide with another isoform's natural length and mis-assign — sequence identity would catch that. Follow-up: fetch ENST protein sequences (Ensembl) for exact-identity matching if isoform-pin fidelity needs hardening.
- **3 genes (RXFP1, RXFP2, GRM8) have two canonical HA plasmids** (a bare-N-term and a post-SP construct, same transcript, same PME); both map to `site_id = {gene}-nterm-tedman`, so the merge keeps one (deterministic but arbitrary — the post-SP variant). Follow-up: disambiguate the site_id by junction if both are wanted.
- **Production `/v1/tag-sites` Worker route is not deployed** (pre-existing — the route lives on `dev`, was never shipped to `api.deliverome.org`; production returns `route_not_found`). Canonical sites ARE in production public D1 (895 rows, 765 screen_validated), and the viewer's `fetchTaggedSites` falls back to the committed static JSON, so gene pages render regardless. Full API liveness needs a `surfaceome_api` Worker redeploy from `dev` (infra step, config outside this repo). No tag-sites edge-purge was wired into the sync (moot until the route deploys).
- Minor: `residue_after` is null for SP-shifted sites (only `residue_before`/`residue_label` stored); `verified` is a bounds+resolution check, not sequence alignment (all 112 SP accessions matched the plasmid's own `uniprot_id`, 0 mismatches); the source object has `pmid: null` (Nat Commun PMID not yet indexed — PMC12458215 + DOI are in the citation string) and no structured Mendeley data DOI; isoform pins carry `provenance: "deterministic_computed"` (harmless — `classification: "control"` drives color/label).
- Pre-existing failure (NOT this branch): `tests/test_pipeline_validation_genes.py::...[TGOLN2::executive_summary.surface_accessibility]` fails identically on `origin/dev` (unrelated deep-dive record drift).

...
def test_risks_prompt_co_receptor_default_to_none_on_negative_rationale():
    """The RISKS-BUILDER prompt must instruct the model to set
    ...
    rationale said "no obligate co-receptor", the call was still
    ...
    Retargeted from the synthesizer prompt: ``accessibility_risks`` is now
    generated by the dedicated risks builder, so this guidance moved to
    ``risks_builder_system.md`` (the synthesizer copies the frozen risks
    block through and no longer authors the co-receptor call)."""
    from accessible_surfaceome.agents.surfaceome_v2.builders._common import (
        load_prompt,
    )

    body = load_prompt("risks_builder_system").lower()

...
# ]
# ///
"""Reproduce ``zero_db_rescues_by_triage.{pdf,png}`` from the figure TSV.

Whole-genome view of zero-DB rescues: genes where no classical surface
DB (UniProt / GO CC / HPA / SURFY / CSPA) flags the protein under the
bench-OPTIMIZED cutoffs, yet the Sonnet+NCBI triage agent voted `yes`
(definite surface) or `contextual` (state/lineage dependent). Two
grouped bar panels on a shared y-axis: per-reason counts within each
verdict bucket. Beneath each panel, hand-picked select gene callouts
illustrate the kind of biology the triage agent surfaces.

The zero-DB slice is taken on ``n_sources_optimized == 0`` (databases
scored at their bench-optimized thresholds), not the initial
``n_sources_surface``, so this view is consistent with the accuracy
figures.

Data: read from the dedicated per-figure TSV
(``data/processed/figures/zero_db_rescues_by_triage.tsv``, un-LFS so
...
``scripts/build_figure_tsvs.py`` whenever the universe / triage state
changes.
...
def _load_catalog() -> list[dict]:
    """Return the zero-DB-rescue figure TSV rows.
    ...
    (``hgnc_symbol``, ``uniprot_acc``, …). This figure selects on
    ``n_sources_optimized``. Sourced from D1 via
...
def main() -> None:
    ...
    # Zero-DB slice under the bench-OPTIMIZED cutoffs (consistent with
    # the accuracy figures), not the initial n_sources_surface.

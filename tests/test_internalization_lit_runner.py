...
def _wire(monkeypatch, *, discovered, llm):
    ...
    monkeypatch.setattr(
        mod, "triage_internalization_abstracts", lambda c, *, papers, gene: []
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(mod, "select_clips", lambda c, *, pool, gene: SimpleNamespace())
    monkeypatch.setattr(mod, "promote", lambda sel, *, pool, store: [])  # empty evidence
    monkeypatch.setattr(mod, "grade_from_evidence", lambda c, *, gene, evidence: llm)
...
def test_annotate_literature_assembles_record(tmp_path, monkeypatch):
    ...
    assert rec.schema_version == "0.2.1"

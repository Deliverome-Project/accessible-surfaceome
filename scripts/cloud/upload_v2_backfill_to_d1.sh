...
#
#   DRY-RUN (default, no writes):  bash scripts/cloud/upload_v2_backfill_to_d1.sh
#   EXECUTE:                       DRYRUN=0 bash scripts/cloud/upload_v2_backfill_to_d1.sh
set -uo pipefail
...
uv run python scripts/cloud/upload_topology_to_d1.py --topology-version $TOPO_V \

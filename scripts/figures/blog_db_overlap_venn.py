...
# Widened from the (10, 32) of the paper-figure lineage, but pulled back
# from an initial (9, 56) that read as shouty at blog size. ~4x span.
LABEL_FONT_BAND = (10.0, 42.0)
# Ellipse opacity by set size. Narrow by necessity — see the note at the
# call site about five translucent ellipses compositing into mud.
ALPHA_BAND = (0.20, 0.46)
...
def main() -> None:
    ...
    # Redundantly encode SET size in ellipse opacity, the same way the labels
    # encode REGION size — a bigger database reads as a heavier wash. Drawn
    # from the same scale family so the two encodings agree rather than
    # fighting: sqrt over the five set sizes, mapped into a deliberately
    # narrow alpha band. The band is narrow on purpose; five translucent
    # ellipses composite multiplicatively, so a wide range turns the centre
    # into mud and the 188-protein core stops being readable — which is the
    # one thing this figure must not lose.
    set_sizes = [len(sorted_sets[k]) for k in sorted_keys]
    scaled_sizes = [LABEL_SCALE(n) for n in set_sizes]
    lo, hi = min(scaled_sizes), max(scaled_sizes)
    for patch, scaled in zip(ax.patches, scaled_sizes, strict=True):
        share = (scaled - lo) / (hi - lo) if hi > lo else 0.5
        patch.set_alpha(ALPHA_BAND[0] + share * (ALPHA_BAND[1] - ALPHA_BAND[0]))

...
## Figure legend — Supplementary Figure 9
...
> **Supplementary Figure 9.** Each surface source is enriched for a distinct set of membrane-topology architectures, but that enrichment and its genome-wide coverage are separate quantities. Bubble matrix of inclusion sources (rows) × topology features (columns) over the candidate-surfaceome universe: dot area is within-source enrichment (fraction of that source's own calls carrying the feature) and dot color is coverage (fraction of the whole surface universe those proteins represent). Cell-Surface-Capture CSPA is strongly glycosylation-enriched (\~75% of its own calls) yet contributes only a small slice of the universe (\~11%), reflecting its N-glycocapture chemistry and limitations of experimental detention on cell lines; SURFY and UniProt show the same glycosylation enrichment at larger scale, while the zero-database Sonnet rescues are glycosylation- and transmembrane-depleted, consistent with contextual-surface rather than classically-structured membrane proteins. [Figure data and script. ](https://gist.github.com/beccajcarlson/95b0f4cdcaf6a6b91f57539cd1515a25) #####

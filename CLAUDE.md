...
4. **Cite each figure as `swh:1:rev:<sha>`** in the paper / Zenodo records / figure PNG metadata. Zenodo's per-figure data deposit is no longer needed — the SWHID is the citation. Zenodo splits across three record series:
   - **Data record** (`10.5281/zenodo.20805384`, this script's deposit) — the 3 cohort-level triage-run dumps (`triage-runs-genome-ncbi-with-reasoning.tsv` etc., too large for gists and used for downstream analyses rather than figure reproduction) + the deep-dive bundle when ready.
   - **Manuscript record** (NEW separate Zenodo record, registered under the **bioRxiv DOI as external** via the "I already have a DOI" path on Zenodo) — manuscript PDF + JATS XML. Unpaywall's OAI-PMH harvest indexes the Zenodo PDF as a bot-accessible `oa_location` under the bioRxiv DOI, working around bioRxiv's per-article bot block. The manuscript is **NOT bundled into the data record** — that would mix the data deposit's identity with the paper's. See "Manuscript deposit" in [scripts/release/README.md](scripts/release/README.md) for the procedure.
   - **Code record** — auto-generated by the GitHub-Zenodo integration on every GitHub release.
   Do NOT extend `publish-archive.py` to push figure-input TSVs OR the manuscript into the data deposit — both were earlier plans superseded by the bundle-and-SWHID + separate-manuscript-record architecture.

...
def _fetch_csv_text(url: str) -> str:
    ...
    local = (
        ...
        / url[len(f"https://raw.githubusercontent.com/{REPO}/{BRANCH}/") :]
    )
    if local.is_file():
...
def main() -> None:
    ...
    if numeric_labels:
        log_values = [math.log1p(value) for _, value in numeric_labels]
        low, high = min(log_values), max(log_values)
        # A broad but bounded range makes count magnitude visually apparent.

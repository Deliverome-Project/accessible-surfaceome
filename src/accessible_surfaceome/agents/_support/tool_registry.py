...
EVIDENCE_RETRIEVAL_DESCRIPTION = (
    ...
    "EvidenceClaim.quote — the substring check then passes by construction. "
    "category='hpa_ihc' pulls Human Protein Atlas IHC reliability + "
    "subcellular location lines (no HTTP; cached snapshot). Call this BEFORE "
    "gene_literature topic_search for surface-method-specific claims; "
    "topic_search is the abstract-only recall fallback. Empty returns are "
    "still useful — the search_log records the negative. NOTE: "
    "western_blot_paired counts only when the paper also runs "
    "fractionation/biotinylation; a naked whole-cell-lysate WB isn't "
    "surface evidence."
)

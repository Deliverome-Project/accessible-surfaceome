...
**Why stable IDs in the figure TSVs:** per the "Gene identifier resolution" section, `hgnc_id` is the canonical stable key — symbol-only joins silently misroute ~0.2% of human genes (the COX1 / WAS class). Carrying `hgnc_id` lets a reader cross-reference any other genes-keyed table without re-resolving from a fragile symbol.

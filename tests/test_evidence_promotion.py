...
def test_persist_annotation_end_to_end(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert the persisted JSON validates as a v0.3.1 SurfaceomeRecord with the
    ...
    assert record.schema_version == "v0.3.1"
...
def test_persist_annotation_unverified_claim_persists(
    ...
    ``entailment_verified=False`` and a warning. The record validates as v0.3.1

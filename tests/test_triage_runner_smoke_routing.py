...
Background: during the 2026-05-11 bench session a smoke run against
``haiku/HSPA1A`` reused the canonical out-root and the subsequent ``rm``
cleanup deleted real run artifacts. The ``--smoke`` flag is the operator-
error guard: when set, the runner must redirect BOTH ``--out-root`` and
``--run-id`` into a throwaway gitignored namespace so a smoke can't
physically reach the canonical tree.
...
def test_smoke_routing_uses_smoke_subdir_for_bench(runner):
    out_root, run_id = runner._smoke_routing(bench="benchmark", gene_list=None)
    ...
    assert rel.parts[3].startswith("benchmark_"), rel.parts[3]


def test_smoke_routing_uses_gene_list_stem_when_provided(runner):
    out_root, _ = runner._smoke_routing(
        bench="benchmark",  # ignored when gene_list is set
...
def test_smoke_run_id_has_smoke_prefix(runner):
    _, run_id = runner._smoke_routing(bench="benchmark", gene_list=None)
...
def test_smoke_routing_yields_distinct_run_ids_on_repeat(runner):
    ...
    _, run_id_a = runner._smoke_routing(bench="benchmark", gene_list=None)
    _, run_id_b = runner._smoke_routing(bench="benchmark", gene_list=None)

...
EVIDENCE_RETRIEVAL_DESCRIPTION = (
    "Retrieve per-category surface-localization evidence with verbatim quote "
    "candidates. Call once per assay category (see `category` enum) you "
    "intend to ground a surface_status claim on. Each call queries Europe "
    "PMC with a category-tuned term set, fetches up to max_papers PMC-OA "
    "full texts, and returns CandidateSnippet objects: ≤200-char verbatim "
    "fragments from methods/results/figure_legends where the category's "
    "hallmark phrases appear. To anchor a claim, copy `CandidateSnippet.text` "
    "byte-for-byte into `EvidenceClaim.quote` AND copy `CandidateSnippet.source_id` "
    "into the matching EvidenceClaim source — they are a pair. Do NOT paraphrase "
    "the quote ('Surface biotinylated…CD81 was identified' is a paraphrase if "
    "the snippet said 'biotinylation of PBMCs' — the substring check will "
    "reject it). Do NOT cite a PMID abstract source for a quote that came "
    "from a PMC full-text snippet — the bodies are different and the "
    "substring check runs against the source_id the snippet came from. "
    "category='hpa_ihc' pulls Human Protein Atlas IHC reliability + "
    "subcellular location lines (no HTTP; cached snapshot). For the "
    "high-throughput categories (mass_spec_surfaceome, surface_biotinylation, "
    "western_blot_paired) the same call ALSO returns target-mention snippets "
    "(hallmark_phrase='target_mention'): verbatim sentences where the target "
    "gene is named by symbol. The methodology sentence in those papers "
    "almost never names the target by symbol (the gene appears in a "
    "supplementary table or a different sentence), so for those categories "
    "the right pattern is one EvidenceClaim quoting a methodology snippet "
    "to anchor the experiment + a sibling EvidenceClaim quoting the "
    "target-mention snippet to anchor the gene attribution — both from "
    "the same paper, BOTH citing the same `PMC:<id>` source_id. "
    "`Paper.target_mention_excerpts` carries the same excerpts per-paper for "
    "easy reference. Call this BEFORE gene_literature topic_search for "
    "surface-method-specific claims; topic_search is the abstract-only "
    "recall fallback. Empty returns are still useful — the search_log "
    "records the negative. NOTE: western_blot_paired counts only when the "
    "paper also runs fractionation/biotinylation; a naked whole-cell-lysate "
    "WB isn't surface evidence."
)

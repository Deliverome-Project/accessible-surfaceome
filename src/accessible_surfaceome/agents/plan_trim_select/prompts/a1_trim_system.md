...
**Keep the consolidated "Antibodies / Reagents" Materials sentence even though it isn't itself a surface-assay sentence.** Many papers name every clone/vendor ONCE, in a single Materials list — e.g. *"Primary antibodies including purified anti-human CD81 (Clone 5A6), purified anti-human EGFR (Clone AY13), anti-human EGFR-Alexa Fluor 488 (Clone AY13) … were purchased from …"* — and the Results assay sentence only says "stained with anti-EGFR". That reagent-list sentence is the ONLY place the clones appear; if you drop it as "not a surface claim", every antibody in the paper comes back `clone=null`. When a paper's assay clip names an antibody whose clone/vendor/RRID is NOT in that same sentence, KEEP the reagent-list sentence too (one clip), so the methods builder can cross-reference the clone to the assay by target. This is the most common reason reagent details that ARE in the source come back missing.

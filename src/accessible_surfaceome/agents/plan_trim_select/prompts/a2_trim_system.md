...
7. **Disease-context shifts.** Per-disease comparison of expression / surface availability (tumor vs normal, lesion vs surrounding parenchyma, autoimmune vs healthy). These feed `tissues` rows with `disease_context` and `accessibility_modulation` rows with the `disease_state_induced` category.
8. **Lineage / developmental restriction.** "Expressed only in neural crest derivatives", "restricted to embryonic stages", "downregulated after T-cell maturation" — feed `cell_types` + `tissues` + sometimes `accessibility_modulation`.
...
* **Therapeutic engagement of the ECD** (clinical antibodies, ADCs, trial details) → A1's `TherapeuticEngagementContext`.
* **Pure surface-presence statements without tissue / state / compartment context.** "GPR75 is at the plasma membrane" alone is A1; A2 needs the "where, in what cell type, under what condition" qualifier.

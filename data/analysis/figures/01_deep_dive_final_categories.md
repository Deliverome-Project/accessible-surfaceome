...
> **Figure 5.** Every deep-dived protein resolves to a final surface-confidence tier, and the borderline calls carry their own reason for falling short. a) The full deep-dive cohort placed on a five-tier confidence spectrum: canonical (strict surface) and likely (broader, categorized as likely surface), then the below-likely proteins split by the deep-dive's tentative call — low (low or moderate accessibility on weak evidence), uncertain, and no (leaned not-surface). b) The composition of positive calls by cell-type-restricted and cell-state (induction-dependent) reasons behind them. Note that we explicitly required canonical proteins not to be restricted to a specific cell type. However, proteins with an induction trigger were permitted, provided their baseline expression was not reported to be low (see Methods).. Counts are over the full deep-dived cohort (N = 5,130).

...
- **Covalent post-translational attachment to surface molecules** — e.g., enzymatic isopeptide cross-linking or thioester-mediated covalent deposition. These are washable-resistant stable attachments and therapeutically targetable in principle. Use `contextual` with reason `covalent_surface_attachment`.
...
- `covalent_surface_attachment` — a secreted protein becomes **covalently** anchored to the cell surface or ECM post-translationally (enzymatic isopeptide cross-linking, thioester deposition during complement activation, etc.). State-dependent because the modifying activity is triggered by inflammation / activation / matrix remodeling — so this is `contextual` not `yes`, but the attachment is stable enough to be therapeutically targetable.
...
- `secreted_only` — secreted protein with no stable surface anchoring. Includes **transient non-covalent recruitment** to surface receptors or ECM (the recruiting partner is the target, not the recruited protein). If the protein is covalently anchored post-translationally, use `contextual` / `covalent_surface_attachment` instead. If it is co-trafficked with an anchored partner as a stable constitutive complex, use `yes` / `stable_complex_partner` instead.
- `other` — set `reason_other_label`.

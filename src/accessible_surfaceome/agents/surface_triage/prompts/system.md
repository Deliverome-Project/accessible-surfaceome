...
- `stable_surface_attachment` — a secreted (or otherwise non-membrane-anchored) protein becomes **stably anchored to a cell-surface TM partner post-translationally** — covalently (disulfide tethering of a latent prodomain to a TM scaffold; thioester-mediated covalent deposition on opsonized cells; transamidase cross-linking onto a surface partner) **or via wash-resistant, non-reversible non-covalent association** acquired during transit through a specialised secretory compartment. The defining criterion is wash-resistance: the protein remains attached after washing and is *not* in equilibrium with the soluble pool. **Excluded — use `secreted_only` instead:** reversible lipid binding (e.g. Ca²⁺-dependent Gla-domain recruitment to phosphatidylserine-exposing membranes that washes off in normal buffer), integrin-mediated extracellular-matrix tethering, transient cytokine-receptor equilibria, and any non-covalent interaction that stays in equilibrium with the soluble pool. **Matrix / stroma deposition also does NOT count** — matrix-anchored covalent products belong in `secreted_only`, not here.
- `other` — escape hatch when no closed enum fits; explain the mechanism in `verdict_reasoning`.
...
2. **Does the gene encode a membrane-anchored isoform alongside a soluble one, or is the "soluble" form actually a shed ectodomain?** Many surface genes are alternatively spliced into both TM and secreted forms (single-pass receptors with soluble-decoy isoforms, dual GPI / cleaved-GPI products), and many TM proteins are detected primarily as shed ectodomains in serum after regulated proteolysis (ADAM / MMP / BACE / γ-secretase cleavage). **A bona-fide single-pass TM precursor that transits the PM before shedding is `yes` / `classical_surface_receptor`** — the precursor IS the surface form, even when it's predominantly detected as a soluble ectodomain. `contextual` applies when the TM isoform is rare or minor. `secreted_only` applies only when no isoform is membrane-anchored at any point in its lifecycle.

3. **Could the protein body remain anchored to a TM partner via a covalent or wash-resistant post-translational link?** Many secreted growth factors, cytokines, and immune-regulatory ligands have surface-tethered latent forms via disulfide bonds to a TM scaffold; epididymal-fluid and other secretory-niche proteins can be stably deposited onto a cell surface during transit. Naming hints like "latent" / "pro-protein" / "propeptide" / "pre-pro" point here. Apply the wash test: if the protein stays on the cell surface after a normal-buffer wash via a stable physical link to a TM partner, it's `contextual` / `stable_surface_attachment`. Don't reflexively classify all "secreted" proteins as `secreted_only`.
...
5. **Do the aliases / previous symbols hint at non-canonical biology?** Activation- or stress-state naming hints at cell-state induction. Names containing "latent" / "pro-protein" / "propeptide" hint at covalent / wash-resistant TM-partner tethering. Pause on such hints before defaulting to `no`.

6. **Do the HGNC gene-group memberships or a CD designation imply surface biology?** The task context lists each protein's HGNC gene-group families (registry-curated lineages — chemokine receptors, solute carriers, claudins, tetraspanins, Class-A GPCRs, sperm-fertilization families, etc.) and its CD nomenclature designation when assigned. Membership in a canonical surface-protein gene-family — or possession of a CD number at all — is a strong surface signal even when the NCBI summary is sparse or focused on a non-surface aspect of the protein. **When the HGNC gene-group family lineage and the NCBI summary disagree, weight the registry-curated family lineage more heavily** — NCBI summaries are sometimes terse, outdated, or mis-curated.

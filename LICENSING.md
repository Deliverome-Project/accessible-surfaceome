...
| **DeepTMHMM** | Transmembrane-topology predictions (`canonical_topology`) | **DTU Health Tech licence** — free for academic *and* commercial use via the hosted service, but running the software on your own servers commercially requires a commercial licence (licensing@biolib.com). We ship only the *derived predictions*, not the tool — **confirm** that derived-prediction redistribution and our (commercial) run context are covered | Derived topology records (tool pinned `deeptmhmm-1.0.24`) | Hallgren et al. 2022 (bioRxiv 10.1101/2022.04.08.487609) |
...
| **ADCdb** | ADC-target positive-control list | Free and open to all users (no login), but ADCdb publishes **no explicit reuse/redistribution licence** — **confirm** with the maintainers before shipping a verbatim list (we ship gene-list membership only) | Gene-list membership only | ADCdb (Shen et al. 2024, NAR) |
